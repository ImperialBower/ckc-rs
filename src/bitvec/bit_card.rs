...
/// use pokerhand::types::bitvec::bit_card::BitCard;
///
...
impl From<PlayingCard> for BitCard {
    fn from(playing_card: PlayingCard) -> Self {
        BitCard::from(playing_card.as_poker_card())
    }
}
...
/// ```
/// use std::convert::TryFrom;
/// use pokerhand::types::bitvec::bit_card::{AnnotatedBitCard, BitCard};
///
...
mod bit_card_tests {
    ...
    fn from_u64__comprehensive_too() {
        let standard52 = cardpack::Standard52::default();
        for card in standard52.deck {
            let actual = BitCard::from(PlayingCard::from(&card).as_poker_card());
    ...
    fn to_poker_card() {
        ...
        for card in standard52.deck {
            ...
            assert_eq!(
                bit_card.to_poker_card(),
                PlayingCard::from(&card).as_poker_card()
            );
        }
    }
    ...
    fn get_suit_binary_signature(#[case] index: &'static str, #[case] expected: u32) {
        let card: cardpack::Card = cardpack::Standard52::card_from_index(index);
